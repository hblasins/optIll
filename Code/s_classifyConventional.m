% This is the main classification script for conventional camera emulated
% images. We evaluate different ambient illuminants (blackbody radiators at
% different temperatures) and classifiers and cross-validate for tuning 
% parameters. 
%
...
fNames = fullfile(imagePath,target,'Conventional','*.mat');
...
%  Classify for every conventional camera type, cross validate for
%  different settings of validation parameters.
...
for f=1:length(fileNames);
    ...
    fName = fullfile(imagePath,target,'Conventional',fileNames(f).name);
    [~, condition] = fileparts(fName);
    
    loadedData = load(fName);
    
    % Load and assemble the data.
    data = zeros(length(x)*(2*delta+1)^2,3);    
    for i=1:length(x)     
        subImg = loadedData.Img(y(i)-delta:y(i)+delta,x(i)-delta:x(i)+delta,:);
        subData = reshape(subImg,[(2*delta+1)^2 3]);
        data((i-1)*(2*delta+1)^2+1:i*(2*delta+1)^2,:) = subData;        
    end    
    
    for c=1:length(classifierNames)
    
        fName = fullfile(resultPath,target,'Conventional',sprintf('%s_%s.mat',condition,classifierNames{c}));
        if exist(fName,'file'),
