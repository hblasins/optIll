...
switch parm
   ...
   case {'photons'}
       ...
       % If present add the effect of a global fluorescence
       if checkfields(scene,'fluorescence')
           illPhotons = sceneGet(scene,'illuminant photons');
           wave = sceneGet(scene,'wave');
           deltaL = wave(2) - wave(1);
           
           % Set the same wavelength first
           scene.fluorescence = fluorescenceSet(scene.fluorescence,'wave',wave);
           absorption = fluorescenceGet(scene.fluorescence,'absorption');
           gain = deltaL*absorption(:)'*illPhotons;
           emission = fluorescenceGet(scene.fluorescence,'emission photons');
           flSpectrum = gain*emission;
           
           fluorescenceEffect = repmat(shiftdim(flSpectrum,-2),sceneGet(scene,'rows'),sceneGet(scene,'cols'));
           
           val = val + fluorescenceEffect;
       end    
       
       % If there is a fluorescence scene, add its contributions
       if checkfields(scene,'fluorescenceScene')
           
           scene.fluorescenceScene = fluorescenceSceneSet(scene.fluorescenceScene,'illuminant',sceneGet(scene,'illuminant'));
           val = val + fluorescenceSceneGet(scene.fluorescenceScene,'photons');
           
       end 
       
       % If the scene is submerged, compute the effect
       
       if checkfields(scene,'dirComp') && checkfields(scene,'scatterComp')         
           val = scene.dirComp + scene.scatterComp;
       end
    ...
    case {'fluorescence'}
        if checkfields(scene,'fluorescence'), val = scene.fluorescence; end
        
    case {'medium'}
        if checkfields(scene,'medium'), val = scene.medium; end
        
    case {'submergeddepth'}
        if checkfields(scene,'submergedDepth'), val = scene.submergedDepth; end
        
    case {'depthdiscretization'}
        if checkfields(scene,'depthDiscretization'), val = scene.depthDiscretization; end
